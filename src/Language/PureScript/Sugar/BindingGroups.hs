  ...
  cycleErrors (ValueDeclarationData (ss, _) n _ _ e) =
    case (e, stripAbs e) of
      ...
      (_, TypeClassDictionaryConstructorApp _ (getFields -> Just fields)) ->
        let fields' = getDictReferencingFields n fields
        in case fields' of
          [] -> errorMessage' ss $ CycleInDeclaration n
          _ -> foldMap (fieldCycleError n) fields'
      _ -> errorMessage' ss $ CycleInDeclaration n

  fieldCycleError :: Ident -> (PSString, Expr) -> MultipleErrors
  fieldCycleError ident (psString, expr) = case separateValAndType expr of
    (expr', mSt) -> errorMessage' (getSourceSpan expr') error'
      where
      ident' = Ident $ prettyPrintString psString
      getError (Just st) = if isFunctionType st then DictMissingEtaExpansion else DictCycleInDeclaration
      getError Nothing = DictCycleInDeclaration
      error' = getError mSt ident ident'

  getDictReferencingFields :: Ident -> [(PSString, Expr)] -> [(PSString, Expr)]
  getDictReferencingFields ident = mapMaybe go
    where
    go :: (PSString, Expr) -> Maybe (PSString, Expr)
    go pair@(_, expr) = if refersToIdent ident expr then Just pair else Nothing
